...
namespace Go_WinApp
{
	...
	public class GoBoard : System.Windows.Forms.Form
	{
		...
		int nSize;		                //棋盘大小为19
		const int nBoardMargin = 10;	//边界大小为10
		int nCoodStart = 4;
		const int	nBoardOffset = 20;
		int nEdgeLen = nBoardOffset + nBoardMargin;
		int nTotalGridWidth = 360 + 36;	//长度和宽度
		int nUnitGridWidth = 22;		//单个格子长度和宽度
		int nSeq = 0;
		Rectangle rGrid;		    //模板
		StoneColor m_colorToPlay;   //颜色
		GoMove m_gmLastMove;	    //上一步操作 
		Boolean bDrawMark;	        //标记 
		Boolean m_fAnyKill;	        //旗子被kill
		Spot [,] Grid;		        //棋盘
		Pen penGrid, penStoneW, penStoneB,penMarkW, penMarkB;
		Brush brStar, brBoard, brBlack, brWhite, m_brMark;
	
        // ZZZZ ZZZZZZZZZ
        int nFFMove = 10;   //最长能返回10次 
        int nRewindMove = 10;  // 清空后保留10次
		...
		{
			GoMove gm = gameTree.doPrev();	//回溯到上一步
            if (null != gm)
            {
                playPrev(gm);
            }
		...
		//判断点击的位置是否closeEnough
        //限定条件是距离nUnitGridWidth*1/3
	...
	public class Spot 
	{
		...
		private Boolean bUpdated; //点是否被更新过
		/**
		 * 初始化
		 */
		public Spot() 
	...
	public class GoMove 
	{
		...
		ArrayList		m_alLabel; //标签的集合 
		ArrayList		m_alMark; //标记的集合
	...
	public class GoTree 
	{
		...
		Boolean parseFile(String goStr) 
		{
			...
			while (iEnd < goStr.Length) 
			{
				...
				if (iEnd < 0) //如果不是";"
					iEnd = goStr.LastIndexOf(")", goStr.Length);		//寻找")"
				if (iBeg >= 0 && iEnd > iBeg) 
				{
		...
		{
			Boolean fMultipleMoves = false;   //是否移动 
			GoMove gm = null; 
			...
			for (int i = 0;i<arrKV.Count;i++)
			{
				...
				for (int j=0; j<((KeyValuePair)(arrKV[i])).alV.Count; j++)
				{
                    ...
                    else if (key.Equals("AW"))		//快捷键"AW"
                    {
                        fMultipleMoves = true;
                        gm = new GoMove(strValue, StoneColor.white);
                    }
                    else if (key.Equals("AB"))		//快捷键"AB"
                    {
                        fMultipleMoves = true;
                        gm = new GoMove(strValue, StoneColor.black);
                    }
